...
describe('Question model', () => {
  ...
  describe('silentFetchQuestion', () => {
    test('should return the question without modifying views', async () => {
      const question = QUESTIONS.find(q => q._id?.toString() === '65e9b5a995b6c7045a30d823');
      mockingoose(QuestionModel).toReturn(question, 'findOne');
      QuestionModel.schema.path('answers', Object);

      const result = (await silentFetchQuestion(
        '65e9b5a995b6c7045a30d823',
      )) as PopulatedDatabaseQuestion;
      ...
      expect(result.text).toEqual(question?.text);
      expect(result.answers).toEqual(question?.answers);
      expect(result.views).toEqual(question?.views);
    });

    test('should return error if question not found', async () => {
      mockingoose(QuestionModel).toReturn(null, 'findOne');

      const result = await silentFetchQuestion('65e9b716ff0e892116b2de01');

      expect(result).toEqual({ error: 'Error when fetching a question' });
    });
  });
});
