...
describe('Test questionController', () => {
  ...
  describe('GET /getQuestionSilent/:qid', () => {
    it('should return a question object in the response when the question id is valid', async () => {
      const qid = '65e9b5a995b6c7045a30d823';

      const mockQuestion2 = MOCK_POPULATED_QUESTIONS.find(q => q._id.toString() === qid)!;

      jest.spyOn(questionUtil, 'silentFetchQuestion').mockResolvedValueOnce(mockQuestion2);

      const response = await supertest(app).get(`/question/getQuestionSilent/${qid}`);

      expect(response.status).toBe(200);
      expect(response.body).toEqual(simplifyQuestion(mockQuestion2));
    });

    it('should return 400 if the question id format is invalid', async () => {
      const invalidQid = 'invalid-id';

      const response = await supertest(app).get(`/question/getQuestionSilent/${invalidQid}`);
    ...
    it('should return 500 if question is not found', async () => {
      const qid = '65e9b5a995b6c7045a30d824';

      jest.spyOn(questionUtil, 'silentFetchQuestion').mockResolvedValueOnce({ error: 'test' });
    ...
    it('should return 500 if an internal server error occurs', async () => {
      const qid = '65e9b5a995b6c7045a30d825';

      jest
        .spyOn(questionUtil, 'silentFetchQuestion')
        .mockRejectedValueOnce(new Error('Unexpected DB failure'));

      const response = await supertest(app).get(`/question/getQuestionSilent/${qid}`);

      expect(response.status).toBe(500);
      expect(response.text).toBe('Error when fetching question by id: Unexpected DB failure');
    });
  });
});
