...
import { DatabaseChat, PopulatedDatabaseChat, Message, DatabaseMessage } from '../../types/types';
...
const getMessageByIdSpy = jest.spyOn(messageService, 'getMessageById');
const updateMessageSpy = jest.spyOn(messageService, 'updateMessage');
...
describe('Chat Controller', () => {
  ...
  describe('POST /chat/:chatId/addMessage', () => {
    ...
    it('should return 500 if populateDocument throws an error', async () => {
      ...
      saveChatSpy.mockResolvedValue(chatResponse);
      populateDocumentSpy.mockRejectedValue(new Error('populate failed'));

      const response = await supertest(app).post('/chat/createChat').send(validChatPayload);

      expect(response.status).toBe(500);
      expect(response.text).toBe('Error creating a chat: populate failed');
    });
  ...
  describe('Socket handlers', () => {
    ...
    const mockUpdatedMessage: DatabaseMessage = {
      _id: new mongoose.Types.ObjectId(),
      msg: 'Hello',
      msgFrom: 'User1',
      msgDateTime: new Date('2024-06-04'),
    ...
    it('should join a chat room when "joinChat" event is emitted', done => {
      ...
      updateMessageSpy.mockResolvedValue(mockUpdatedMessage);
