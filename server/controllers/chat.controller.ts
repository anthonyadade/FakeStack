...
import { getMessageById, saveMessage, updateMessage } from '../services/message.service';
...
const chatController = (socket: FakeSOSocket) => {
  ...
  const createChatRoute = async (req: CreateChatRequest, res: Response): Promise<void> => {
    ...
    const { participants, messages, subscriptions } = req.body;
  ...
  socket.on('connection', conn => {
    conn.on('joinChat', async (chatID: string | null, username: string) => {
      if (chatID) {
        conn.join(chatID);
        ...
        await Promise.all(
          chat.messages.map(async msg => {
            const message = await getMessageById(msg);
            if ('error' in message) {
              throw new Error(message.error);
            }
            if (!message.readBy.includes(username) && message.msgFrom !== username) {
              message.readBy.push(username);
            }
            const updatedMessage = await updateMessage(message._id, message);
