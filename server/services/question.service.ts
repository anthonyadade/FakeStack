...
/**
 * Fetches a question by ID without incremementing viewcount.
 * @param {string} qid - The question ID
 * @returns {Promise<QuestionResponse | null>} - The question or error message
 */
export const silentFetchQuestion = async (
  qid: string,
): Promise<PopulatedDatabaseQuestion | { error: string }> => {
  try {
    const q: PopulatedDatabaseQuestion | null = await QuestionModel.findOne(
      ...
      { path: 'answers', model: AnswerModel, populate: { path: 'comments', model: CommentModel } },
      { path: 'comments', model: CommentModel },
      { path: 'subscriptions', model: SubscriptionModel },
    ]);
