...
import { Message, PopulatedDatabaseChat, Subscription } from '../types/types';
...
export const sendMessage = async (
  ...
  const notiToAdd = {
    notiSource: String(chatID),
    type: 'message',
    preview: `${message.msg.substring(0, 50)}${message.msg.length > 50 ? '...' : ''}`,
    notiFrom: message.msgFrom,
  };

  res.data.subscriptions.map(async (s: Subscription) => {
    if (s.subscriber !== message.msgFrom) {
...
export const createChat = async (participants: string[]): Promise<PopulatedDatabaseChat> => {
  ...
  await Promise.all(participants.map((p: string) => createSubscription(res.data._id, p, 'chat')));

  // Refetch the new chat so the subscriptions are updated.
  const populatedRes = await getChatById(res.data._id);
