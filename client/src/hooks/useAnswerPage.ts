...
const useAnswerPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchSubscriptionId = async () => {
      try {
        if (!question) return;
        const res: string | undefined = await getUserSubscriptionId(question, user.username);
        setSubscriptionId(res);
      } catch (error) {
        // eslint-disable-next-line no-console
        console.error('Error fetching user subscription for this question:', error);
      }
    };

    // eslint-disable-next-line no-console
    fetchSubscriptionId().catch(e => console.log(e));
