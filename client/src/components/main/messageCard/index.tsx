 ...
 */
const MessageCard = ({ message, user }: { message: DatabaseMessage; user: string }) => {
  const otherReaders = message.readBy.filter(
    username => username !== user && username !== message.msgFrom,
  );
  ...
  return (
    <div className='message'>
      <div className='message-header'>
        ...
        <div>{user && otherReaders.length !== 0 && `Read by: ${otherReaders.join(', ')}`}</div>
      </div>
